...
object DicePreferences {
    ...
    /**
     * Whether the dice and the app accent are kept as one colour.
     *
     * Symmetrical on purpose. There is no leader and no follower: while this is on, changing
     * either one writes both, so whichever of the two somebody happens to be looking at is the
     * one they can drive. A direction would only be worth naming if the two could disagree, and
     * the whole point of the setting is that they cannot.
     *
     * That also means nothing is held in reserve to be restored. Turning it off does not put a
     * colour back — it simply stops the two being written together, and they go their own ways
     * from wherever they both are.
     */
    fun matchColours(context: Context): Boolean =
        prefs(context).getBoolean(KEY_MATCH_COLOURS, false)

    fun setMatchColours(context: Context, match: Boolean) {
        prefs(context).edit().putBoolean(KEY_MATCH_COLOURS, match).apply()
    }
